...
Evaluates the trained BPNN on its held-out test set (15% of trials
set aside during training, never seen by the model).

Loads model.pt and scaler.joblib from the results directory, applies
the same StandardScaler used during training, then runs inference.

Usage:
  python test_model_bpnn.py              # defaults to results_bpnn/
  python test_model_bpnn.py results_bpnn
'''
...
def load(results_dir):
    ...
    return model, scaler, X_test, y_test, meta
...
if __name__ == '__main__':
    results_dir = sys.argv[1] if len(sys.argv) > 1 else 'results_bpnn'

    if not os.path.isdir(results_dir):
        print(f'Error: directory not found: {results_dir}')
        print('Usage: python test_model_bpnn.py [results_bpnn]')
        sys.exit(1)

    for required in ('model.pt', 'scaler.joblib', 'X_test.npy', 'y_test.npy', 'results.json'):
        if not os.path.exists(os.path.join(results_dir, required)):
            print(f'Error: {required} not found in {results_dir}')
            print('Re-run train_model_bpnn.py to regenerate.')
            sys.exit(1)

    print(f'── Testing BPNN in {results_dir}/ ───────────────────')
    model, scaler, X_test, y_test, meta = load(results_dir)
    ...
    y_pred, proba, bal_acc, report, cm = evaluate(model, scaler, X_test, y_test)
    ...
    plot_confusion_matrix(
        cm,
        'Test set confusion matrix (BPNN)',
